...
subjects = ["perlbench", "bzip2", "gcc", "mcf", "milc", "gobmk", "hmmer", "sjeng", "libquantum", "lbm", "sphinx3"]
for path in glob.glob(f'./wrapper-{tool}-{opt}/**/*.0001/', recursive=True):
    ...
    for subject in subjects:
        if subject in path:
            if tool == "softboundcets":
                ...
                os.system(f"time sh ./run_scripts/{subject}.sh CM-SBCETS-{subject} ./workload/{subject}/ref/intput/")
            if tool == "movec":
                ...
                os.system(f"time sh ./run_scripts/{subject}.sh CM-MoveC-{subject} ./workload/{subject}/ref/intput/")
            if tool == "asan":
                ...
                os.system(f"time sh ./run_scripts/{subject}.sh CM-ASAN-{subject} ./workload/{subject}/ref/intput/")
