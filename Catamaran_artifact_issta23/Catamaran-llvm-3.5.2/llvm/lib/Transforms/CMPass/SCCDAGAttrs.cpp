...
bool SCCDAGAttrs::isSCCContainedInSubloop (const LoopSummary &LIS, SCC *scc) const {
  auto instInSubloops = true;
  auto topLoop = LIS.getLoopNestingTreeRoot();
  for (auto iNodePair : scc->getInternalNodePairs()) {
...
void SCCDAGAttrs::collectLoopCarriedDependencies (LoopSummary &LIS) {
...
bool SCCDAGAttrs::checkIfSCCOnlyContainsInductionVariables (
  ...
  for (auto iv : IVs) {
    if (scc->isInternalNode(iv->getLoopEntryPHI())) {
...
bool SCCDAGAttrs::checkIfReducible (SCC *scc, LoopSummary &LIS) {
  ...
  for (auto dependency : this->sccToLoopCarriedDependencies.at(scc)) {
    ...
    if (dependency->isControlDependence()) {
      if (scc->isInternalNode(producer)) return false;
      continue;
    }
     ...
     */
    if (!scc->isInternalNode(consumerPHI)) continue;
...
void SCCDAGAttrs::checkIfClonable (SCC *scc, ScalarEvolution &SE, LoopSummary &LIS) {
...
void SCCDAGAttrs::checkIfClonableByUsingLocalMemory(SCC *scc, LoopSummary &LIS) {
...
bool SCCDAGAttrs::isClonableByHavingNoMemoryOrLoopCarriedDataDependencies (SCC *scc, LoopSummary &LIS) const {
...
void SCCDAGAttrs::iterateOverLoopCarriedDataDependences (
   ...
   */
  for (auto valuePair : scc->getInternalNodePairs()) {
