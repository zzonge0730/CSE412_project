...
LoopCarriedVariable::LoopCarriedVariable (
  ...
  assert(sccContainingVariable.isInternalNode(declarationPHI)
  ...
  for (auto nodePair : sccContainingVariable.getInternalNodePairs()) {
   ...
   */
  for (auto dataOrMemoryNodePair : sccOfDataAndMemoryVariableValuesOnly->getInternalNodePairs()) {
...
bool LoopCarriedVariable::isEvolutionReducibleAcrossLoopIterations (void) const {
  ...
  for (auto controlValue : controlValuesGoverningEvolution) {
    if (sccOfVariableOnly->isInternalNode(controlValue)) return false;
  }
...
PDG *LoopCarriedVariable::produceDataAndMemoryOnlyDGFromVariableDG(PDG &variableDG) const {
  ...
  for (auto nodePair : variableDG.getInternalNodePairs()) {
...
std::unordered_set<Value *> LoopCarriedVariable::getConsumersOfVariable (void) const {
  ...
  for (auto externalNodePair : sccOfVariableOnly->getExternalNodePairs()) {
    ...
    for (auto edge : node->getIncomingEdges()) {
      ...
      if (sccOfVariableOnly->isExternalNode(producer)) continue;
...
bool LoopCarriedVariable::areValuesPropagatingVariableIntermediatesOutsideLoop (std::unordered_set<Value *> values) const {
  ...
  while (!valuesToCheck.empty()) {
    ...
    if (auto cast = dyn_cast<CastInst>(value)) {
      ...
      if (sccOfDataAndMemoryVariableValuesOnly->isExternalNode(valueToCast)) return false;
      ...
      if (sccOfDataAndMemoryVariableValuesOnly->isExternalNode(singleIncomingValue)) return false;
...
PHINode *LoopCarriedVariable::getLoopEntryPHIForValueOfVariable (Value *value) const {
   ...
   */
  if (sccOfVariableOnly->isExternalNode(value)) {
