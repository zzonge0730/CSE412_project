#include "TransferForAsan.h"

#define ENABLELOOP 1

#define ENABLELOOPFREE 0

char TransferForAsan::ID = 0;

static RegisterPass<TransferForAsan> X("TFAsan", "Transfer For Asan in the program");

TransferForAsan::TransferForAsan() : ModulePass{ID} {

}

TransferForAsan::~TransferForAsan() {

}

void TransferForAsan::getAnalysisUsage(AnalysisUsage &AU) const {
...
bool TransferForAsan::runOnModule(Module &M) {
    ...
    for (auto treeIt = trees.rbegin(); treeIt != trees.rend(); ++treeIt) {
        ...
        for (auto loop : loopsToParallelize) {
            ...
            if (ls->getNestingLevel() == 1) {
                ...
                std::unordered_set<Instruction *> instOfLiveInRefinedAndLoadInLoop;
                // get liveIn Vars
                for (auto envIndex : loop->loopEnviroment->getEnvIndicesOfLiveInVars()) {
                    Value * liveIn = loop->loopEnviroment->producerAT(envIndex);

                    if (isa<PtrToIntInst>(liveIn)) {
                        Instruction * nextInst = getNextInstruction(cast<PtrToIntInst>(liveIn), cast<PtrToIntInst>(liveIn)->getParent());
                        if (isa<CallInst>(nextInst) && IsSafeCheckCall(cast<CallInst>(nextInst))) {
                            continue;// filter some false  positive livein vars
                        }
                    }
                        ...
                        for (auto &edge : fromNode->getOutgoingEdges()) {
                            if (edge->isMustDependence() && edge->dataDepToString()=="RAW") {
                                ...
                                if (isa<StoreInst>(toNode) && ls->isIncluded(toNode)) {
                                    ...
                                    instOfLiveInRefinedAndLoadInLoop.insert(toNode);
                                }
                                // if (isa<LoadInst>(toNode) && ls->isIncluded(toNode)) {
                                //     instOfLiveInRefinedAndLoadInLoop.insert(toNode);
                                // }
                            }
                        }
                ...
                //     continue;
                // }
                // for (auto it : instOfLiveInRefinedAndLoadInLoop) {
                //     errs() << "LiveIn RedefAndLoad: " << *it << "\n";
                // }
                ...
                std::unordered_set<Instruction *> brInstRelated(instOfLiveInRefinedAndLoadInLoop.begin(),instOfLiveInRefinedAndLoadInLoop.end()); // 偷懒 instOfLiveInRefinedAndLoadInLoop 中的结果放到了br中判断

                // std::srand(std::time(0)); //seed
                std::unordered_set<Instruction *> safecheckCallInstDoNotInLoopBody;
                // bool coinFlag = true;
                int countCoin = 0; // comment for inital test 
                    ...
                    for (auto& I : *body) {
                        if (CallInst *CI = dyn_cast<CallInst>(&I)) {
                            ...
                            if (IsSafeCheckCall(CI)) {
                                // if (countCoin < 10) {// && IsIntraTaskConsidered(CI)) { // 23
                ...
                // errs() << "vvvvvvallInstsToOneCallInstInLoopBodyvvvvvvvvv\n";
                ...
                if (allInstsToOneCallInstInLoopBody.size() == 0 || 
                safeCheckInstsInLoopBody.size() == 0 || safecheckCallInst.size() == safecheckCallInstDoNotInLoopBody.size()) {
                    delete task;
                ...
                for (auto instcall : safecheckCallInstDoNotInLoopBody) {
                    ...
                    for (auto interninst : allInstsToOneCallInstInLoopBody.at(instcall)) {
    ...
    // errs() << "Final module: \n";
    // errs() << *this->program << "\n";
    errs() << "TransferForAsan::runOnModule...before return\n";
    return false;
}

Function * TransferForAsan::getEntryFunction(void) const {
    return this->program->getFunction("softboundcets_pseudo_main");//"main"
}

std::vector<LoopStructure *> * TransferForAsan::getLoopStructures(void) {
...
std::vector<LoopStructure *> * TransferForAsan::getLoopStructures(Function * func) {
    //check if the function has loops
    auto allLoops = new std::vector<LoopStructure *>();
    //may be should check fun is a declration or not
    auto& LI = getAnalysis<LoopInfo>(*func);
...
std::vector<Function *> * TransferForAsan::getModuleFunctionsReachableFrom(Module * M, Function * startingPoint) {
    auto functions = new std::vector<Function*>();
    auto &callGraph = getAnalysis<CallGraphWrapperPass>();
...
PDG * TransferForAsan::getFunctionDependenceGraph(Function * func) {
    return this->pdgAnalysis->getFunctionPDG(*func);
}

StayConnectedNestedLoopForest * TransferForAsan::organizeLoopsInTheirNestingForest(std::vector<LoopStructure *> const & loops) {
...
DominatorSummary * TransferForAsan::getDominators(Function * f) {
...
std::vector<LoopDependenceInfo *> TransferForAsan::selectTheOrderOfLoopsToParallelize(StayConnectedNestedLoopForestNode * tree) {
...
void TransferForAsan::linkTransformedLoopToOriginalFunction(
    Module * module,
...
bool TransferForAsan::instHappensBefore(Instruction * inst, Instruction * final) {
...
Constant * TransferForAsan::generateJoinFunc() {
...
uint32_t TransferForAsan::calBaselineCost(std::vector<Instruction*> safecheckInsts) {
    uint32_t res = 0;
